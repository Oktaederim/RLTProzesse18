document.addEventListener('DOMContentLoaded', () => {
    ...
    function calculateAll() {
        try {
            ...
            if (inputs.kuehlerAktiv && inputs.kuehlmodus === 'dehumidify') {
                 ...
                 if (zielTaupunkt < (parseFloat(dom.tempKuehlVorlauf.value) || 0) + 2.0) {
    ...
    function addEventListeners() {
        ...
        // Toggles and Selects
        const toggles = [dom.kuehlerAktiv, dom.fanCostActive];
        toggles.forEach(toggle => {
            if(toggle) toggle.addEventListener('change', () => { handleKuehlerToggle(); calculateAll(); });
        });
